...
function filterDanGroupPeople(arr) {
  const filteredPeople = arr.filter(person => person.age < 35);
  const groupedByNationality = _.groupBy(filteredPeople, "nationality");

  return groupedByNationality;
}

console.log(filterDanGroupPeople(people))
